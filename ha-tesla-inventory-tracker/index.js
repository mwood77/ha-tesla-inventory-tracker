...
const sendNotification = async (device, message, vin, model) => {
    ...
    try {
        ...
        logErrorMessage(
            'sendNotification',
            `Sending notification: ${JSON.stringify(payload)}`,
        );
    }
};
...
const loadYamlVersion = () => {
    try {
        ...
        logErrorMessage(
            'loadYamlVersion',
            `Failed to load yaml configuration for version: ${error.message}`,
        );
    }
};
...
const loadConfig = () => {
    ...
    logMessage(
        'main',
        'Configuration loaded with values: ' + JSON.stringify(config) + '\n',
    );
    return config;
};
...
const fetchCpoData = async (config) => {
    ...
    config.models.forEach((model) => {
        ...
        const teslaV4ApiQuery = composeUrlQuery(
            mappedModel,
            config.market,
            config.language,
            config.postalCode,
    ...
    for (const url of constructedUrls) {
        try {
            ...
            logErrorMessage(
                'fetchCpoData',
                `Something went wrong: ${error.message}, #error: ${error}`,
            );
            return [];
        }
    }
    ...
    setInterval(
        async () => {
            ...
            vehicles.forEach(({VIN, price, model}) => {
                db.get(
                    'SELECT price FROM vehicle WHERE vin = ?',
                    [VIN],
                    (err, row) => {
                        if (err) {
                            logErrorMessage(
                                'monitorVins',
                                `Database error for VIN ${VIN} (line 124): ${err.message}`,
                            );
                            return;
                        }
                    ...
                    db.get(
                        ...
                        (err, row) => {
                            if (err) {
                                logErrorMessage(
                                    'monitorVins',
                                    `Database error for VIN ${VIN} (line 155): ${err.message}`,
                                );
                                return;
                            }

                            if (row) {
                                db.run('DELETE FROM vehicle WHERE vin = ?', [
                                    VIN,
                                ]);
                                devices.forEach((device) => {
                                    sendNotification(
...
const logErrorMessage = (functionName, message) => {
    ...
    console.error(
        consoleColours.red,
        `[${currentTime}] [${functionName}] - ERROR - ${message}`,
    );
};
...
const main = () => {
    ...
    logMessage(
        'main',
        'Found a bug? Report it here: https://github.com/mwood77/ha-tesla-inventory-tracker/issues',
        true,
    );
    logMessage('main', '===============================================', true);
